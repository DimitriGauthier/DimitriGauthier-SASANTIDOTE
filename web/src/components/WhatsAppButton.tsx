...
export default function WhatsAppButton({
  ...
  const message = pick(locale, siteConfig.whatsappMessage.fr, siteConfig.whatsappMessage.en);
  return (
    <a
      href={whatsappUrl(digits, message)}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="WhatsApp"
      className="fixed bottom-5 right-5 z-40 flex items-center gap-2 rounded-full bg-primary px-4 py-3 text-sm font-medium text-primary-foreground shadow-lg transition-all duration-300 hover:-translate-y-0.5 hover:brightness-105"
    >
      {/* Logo WhatsApp officiel (glyphe monochrome, prend la couleur du texte). */}
